...
class TrainingParameters:
    ...
    def __init__(self,
                ...
                ,optimizer = "keras.optimizers.Adam" #(learning_rate=3.e-4)" #Adam(learning_rate=3.e-4) #eval(f"keras.optimizers.Adam(learning_rate={learning_rate})")
        ...
        self.loss = loss
        self.optimizer = eval(f"{optimizer}(learning_rate={learning_rate})")
        ...
        if outfolder == '':
            outfolder = Path("../../output") / f"{dataset_name}"/f"{timestr}_fil{n_filter_base}_depth{n_depth}_learning_rate{learning_rate}_scale{scale}_batch{batch_size}"
        ...
        ,loss = loss
        ,optimizer = f"{optimizer}(learning_rate={learning_rate})"
        ,metrics = metrics
        ,n_train = n_train
        ,n_val = n_val
