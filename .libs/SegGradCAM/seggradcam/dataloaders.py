...
class DRIVE:
    """Store a train/test/val subset of the DRIVE dataset in an object"""

    def __init__(self, scale=2, n_classes=2, n=None, prefix="training", shuffle=True, normalize=True,
                 outfile="../../inputs/drive/saved_npz/"):
    ...
    def label_to_classes(self, y):
        y = y.copy()
        # Giả sử lớp 0 là background, lớp 1 là vessel
        y[y > 0] = 1  # Tất cả nhãn > 0 (tức là mạch máu) sẽ được đặt thành lớp 1 (vessel)
        return to_categorical(y, num_classes=self.n_classes).astype(np.bool)
    ...
    def load_from_path(self, path):
        root = Path(path) / self.prefix
        print("Root path: ", root)
        fx = np.array(tuple(sorted(root.glob("*/*images/*.png"))))
        print("Length of the set:", len(fx), ". ", str(self.n), " will be loaded.", flush=True)
        ...
        fx = fx[:self.n]
        fy = tuple(
            str(f).replace("images", "1st_manual").replace(".png", "_mask.png") for f in fx)  # Adjust if needed
...
def get_drive(path, n=None, scale=2, prefix="training", shuffle=True, n_classes=2):
    """Function: Load train/test/val set of DRIVE"""

    def label_to_classes(y):
        y = y.copy()
        # Assign labels to the classes (Assume binary classification here, adapt if necessary)
        y[y > 0] = 1  # Change this line depending on your label processing
        return to_categorical(y, num_classes=n_classes).astype(np.bool)
    ...
    print("Root path: ", root)
    fx = np.array(tuple(sorted(root.glob("*/*images/*.png"))))
    print("Path to images: ", root.glob("*images/*.png"))
    print("Length of the set:", len(fx))
    ...
    fx = fx[:n]
    fy = tuple(str(f).replace("images", "1st_manual").replace(".png", "_mask.png") for f in fx)
    X = np.stack(tuple(map(lambda f: preproc(zoom(imread(f), (1. / scale, 1. / scale, 1), order=1)), tqdm(fx))))
    Y = np.stack(tuple(map(lambda f: label_to_classes(zoom(imread(f), (1. / scale, 1. / scale), order=0,
                                                           prefilter=False)), tqdm(fy))))
