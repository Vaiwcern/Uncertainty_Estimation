...
if __name__ == "__main__":
    ...
    for images, masks in test_dataset:
        print("\n-----------------------\nImage shape:", images.shape, "Mask shape:", masks.shape)
    ...
    # model = unet(input_shape=(128, 128, 3), dropout_rate=0.0)
    ...
    checkpoint = ModelCheckpoint(f"{BASE_ROOT}/model_BUI_drop/model_best.h5", monitor='val_loss', save_best_only=True, mode='min')

    history = model.fit(train_dataset,
                        epochs=100,
                        validation_data=test_dataset,
                        callbacks=[checkpoint])
    model.save(f"{BASE_ROOT}/model_BUI_drop/model_final.h5")
