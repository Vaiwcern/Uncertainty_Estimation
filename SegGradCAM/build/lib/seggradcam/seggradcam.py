...
class SegGradCAM:
    ...
    def featureMapsGradients(self):
        grad_model = keras.models.Model(
            self.input_model.inputs, [self.input_model.get_layer(self.prop_from_layer).output, self.input_model.output]
        )

        # img_array = self.get_img_array(self.image_path)
        preprocessed_input = np.expand_dims(self.image, 0)

        # Define the input signature
        # input_signature = [tf.TensorSpec(shape=(1, img_array.shape[1], img_array.shape[2], img_array.shape[3]), dtype=tf.float32)]
        input_signature = [tf.TensorSpec(shape=(1, preprocessed_input.shape[1], preprocessed_input.shape[2], preprocessed_input.shape[3]), dtype=tf.float32)]
